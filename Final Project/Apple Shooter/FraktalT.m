function FraktalT(axes,n,r,phi,xb,yb)
...
% h=figure;
%  axes(handles.axes1,'axes1',h,'Color',[0.1,0.5,0.4]);
global A
mN=length(phi);
if length(r)==1
    rM=ones(1,mN)*r;
...
% initial coordinates   
    ...
    tau=1;
      for i=1:mN:mN^n
          z=1;
           for k=1:1:mN-1
                for j=z:1:n-1
                 line(axes,[A(j,i,1),A(j+1,i,1)],[A(j,i,2),A(j+1,i,2)],'Color',[0.8,0.5,0.5],'LineWidth',10);
                end
              z=z+1;
          end       
     end
 ...
 % -------------------------- end branches ----------------------------    
      for i=1:1:mN^n
        line(axes,[A(n,i,1),A(n+1,i,1)],[A(n,i,2),A(n+1,i,2)],'Color',[0.8,0.5,0.5],'LineWidth',4);
        
      end   
 % -------------- Trunk -----------------         
    line(axes,[xb(1),xb(2)],[yb(1),yb(2)],'Color', [0.8,0.5,0.5],'LineWidth',20);
hold on